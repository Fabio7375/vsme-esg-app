...
st.header("📊 Analisi ESG sintetica") col1, col2 = st.columns(2) with col1: st.metric("% Energia rinnovabile", f"{energia_rinnovabile}%") st.metric("% Donne in azienda", f"{donne}%") with col2: st.metric("% Rifiuti riciclati", f"{riciclo}%") st.metric("Ore di formazione", f"{formazione:.1f}")
...
pdf.save()
buffer.seek(0)
st.download_button("📄 Scarica il PDF", data=buffer, file_name="report_esg_vsme.pdf", mime="application/pdf")
